...
```bash
cd agent
uv run uvicorn chatbot.server:app  # Start backend on port 8080
```

Note: Stop any logfire platform instances to avoid port 8080 conflicts.
...
- **Dev proxy**: `/api` proxied to `localhost:8080`
